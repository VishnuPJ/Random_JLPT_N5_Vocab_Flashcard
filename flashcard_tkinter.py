...
with open('n5_vpj_anki_flashcard.txt', 'w',encoding='utf-8') as file:
    for val in main_lst:
        # print("    ".join(val)+"\n")
        file.write("    ".join(val)+"\n")
...
class FlashcardApp:
    def __init__(self, root):
        ...
        self.show_romaji = tk.BooleanVar(value=True)
        self.show_meaning = tk.BooleanVar(value=True)
    ...
    def create_widgets(self):
        ...
        self.card_frame.pack(padx=40, pady=40)

        self.kanji_label = ttk.Label(self.card_frame, font=("Arial", 40, "bold"))
        self.kanji_label.grid(row=0, column=0, columnspan=2, pady=10)

        self.hiragana_label = ttk.Label(self.card_frame, font=("Arial", 35),foreground="red")
        self.hiragana_label.grid(row=1, column=0, columnspan=2, pady=5)

        self.romaji_label = ttk.Label(self.card_frame, font=("Arial", 14),foreground="gray")
        self.romaji_label.grid(row=2, column=0, columnspan=2, pady=5)

        self.meaning_label = ttk.Label(self.card_frame, font=("Arial", 12),foreground="gray")
        self.meaning_label.grid(row=3, column=0, columnspan=2, pady=5)

        self.toggle_romaji_button = ttk.Checkbutton(self.root, text="Show Romaji", variable=self.show_romaji, command=self.show_card)
        self.toggle_romaji_button.pack(side="left", padx=10)

        self.toggle_meaning_button = ttk.Checkbutton(self.root, text="Show Meaning", variable=self.show_meaning, command=self.show_card)
        self.toggle_meaning_button.pack(side="left", padx=10)

        self.randomize_button = ttk.Checkbutton(self.root, text="Randomize", variable=self.randomize)
        self.randomize_button.pack(side="left", pady=100)

        self.next_button = ttk.Button(self.root, text="Next", command=self.show_next_card)
        self.next_button.pack(side="left", padx=10)
        # self.next_button.place(x=20, y=80)
    ...
    def show_card(self):
        ...
        self.romaji_label.config(text=romaji if self.show_romaji.get() else "")
        self.meaning_label.config(text=meaning if self.show_meaning.get() else "")

    def show_next_card(self):
        if self.randomize.get():
            self.show_next_card_random()
        else:
            self.current_index = (self.current_index + 1) % len(main_lst)
            self.current_card = main_lst[self.current_index]
            self.show_card()

    def show_next_card_random(self):
